...
function setGenre() {
    ...
    allEl.classList.add("genre-link");
    allEl.innerText = "All";
...
function searchMovies() {
    ...
    if (searchTerm) {
        movieList.innerHTML = '';
        const searchUrl = `https://api.themoviedb.org/3/search/movie?${api_Key}&query=${encodeURIComponent(searchTerm)}`;
        getMovies(searchUrl);
        selectedGenre = [];
        activeSelected();
        $(window).unbind('scroll');
...
const showMovies = (data) => {
    ...
    async function showModal(card) {
        ...
        const streamingLink = `https://t.me/CinephilesAlliance`;
            ...
            <div class="right">
                ...
                <div class="description">
                    ...
                    <div class="stream-link">
                        <a href="${streamingLink}" target="_blank" class="watch-now-btn">Watch Now</a>
                    </div>
                </div>
            </div>
        `;
        ...
        datamap.forEach(e => {
            dirEl.innerHTML = `Directed By: ${e.name}`;
        });
        ...
        movieCredits.cast.forEach((e, idx) => {
            if (e.known_for_department == "Acting" && idx <= 4) {
                ...
                castEl.innerHTML = `Cast: ${castData.join(", ")}`;
            }
        });
